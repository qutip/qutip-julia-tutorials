...
Since we are considering unitary dynamics, i.e., no dissipation, the dynamics are fully captured for $\chi t \in \left[0, 2 \pi \right]$, and the coherent initial state is representive for a laser light source. 
...
display(fig2)
```
As expected, the photon number is conserved throughtout. Either the expeactation value or the Fock distribution are conserved in the nonlinear interaction.
