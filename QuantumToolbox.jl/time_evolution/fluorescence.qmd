...
In this tutorial, the usages of `QuantumToolbox.correlation_2op_1t` and `QuantumToolbox.spectrum_correlation_fft` are mainly demonstrated with the example of resonance fluorescence in the weak field limit.

## Introduction

Reconance fluorescence is the radiative behaviour towards the surrounding EM continuum of a two-level atom driven by a resonating light mode in coherent state (usually a monochromic laser beam). The Hamiltonian $H = H_a + H_{drv}$ has that

- $H_a = \frac{\omega_a}{2} \sigma_z$
- $H_{drv} = \Omega cos(\omega_L t) (\sigma^{+} + \sigma^{-})$
...
Note that with $\cos(\omega_L t) = \frac{e^{i\omega_a t} + e^{-i\omega_a t}}{2}$, $H_{drv}$ can be expressed as 
$$
H_{drv} = \frac{\Omega}{2} \Big[(e^{i2\omega_a t} + 1) \sigma^{+} + ( 1 + e^{-i2\omega_a t}) \sigma^{-}\Big]
$$

In the weak field limit, which is the main scope of this tutorial, the ratio between field strength and frequency of the atom is very small ($\Omega/\omega_a \ll 1$). We can thereby apply the [rotating-wave approximation (RWA)](https://en.wikipedia.org/wiki/Rotating-wave_approximation) to drop dynamics whose frequency is to the order of $\omega_a$, including the bare atom Hamiltonian $H_a$ and time dependent part of $H_{drv}$, yielding
$$
H \sim \frac{\Omega}{2} \sigma_x 
$$

### dissipation dynamics with EM continuum

From the standard interaction Hamiltonian with EM continuum $H_{int} = \sum_l \alpha_l b_l \sigma^{+} + \text{h.c.}$, we follow the Born-Markovian approximation to obtain dissipative-part of the dynamics
$$
...
- $\mathcal{D}[\cdot]$ is the [Lindblad dissipator](https://en.wikipedia.org/wiki/Lindbladian)

## Usage


```{julia}
import QuantumToolbox: QuantumObjectEvolution, n_thermal, liouvillian, sigmap, sigmam, sigmax, sigmay,
...
```{julia}
Ω = 1
# we are being generous here to set Ω/ωa = 1e-3, realistically it sould be way smaller despite strong driving
ωa, ωL = 1e3 * Ω, 1e3 * Ω 
γ0 = 0.05
KT = 0
```
...
```{julia}
function liouvillian_H(_Ω, _γ0, _KT)
    Ha = ωa/2 * sigmaz()
    cos_evo(t) = _Ω * cos(ωL * t)
    Hdrv = QuantumObjectEvolution((sigmap() + sigmam(), cos_evo))
    ...
    return liouvillian(Ha + Hdrv, c_ops)
end

function liouvillian_I(_Ω, _γ0, _KT)
...
sol = mesolve(liouvillian_I(Ω, γ0, KT), ψ0, tlist, nothing, e_ops = e_ops)
```
...
for (idx, γ0) in enumerate(γ0s)
    result = mesolve(liouvillian_I(Ω, γ0, KT), ψ0, tlist, nothing, e_ops = e_ops)
